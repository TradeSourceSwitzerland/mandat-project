...
document.addEventListener("DOMContentLoaded", () => {
  ...
  /* ======================== INIT: LOAD DATA ======================== */
  (async () => {
    if (statusText) statusText.textContent = "Lade Leads vom Server…";

    await loadAMTOVZ();

    try {
      const token = localStorage.getItem("auth_token");
      const res = await fetch(API + "/zevix/leads?limit=5000", {
        headers: { "Authorization": `Bearer ${token}` },
        credentials: "include"
      });
      if (!res.ok) throw new Error("HTTP " + res.status);
      const data = await res.json();
      if (!data.success) throw new Error(data.error || "Fehler beim Laden der Leads-Daten vom Server");
      rows = data.leads.map(lead => ({
        Firma: lead.firma || "",
        Rechtsform: lead.rechtsform || "",
        Strasse: lead.strasse ? `${lead.strasse} ${lead.hausnummer || ""}`.trim() : "",
        PLZ: lead.plz || "",
        Ort: lead.ort || "",
        Sitz: lead.sitz || "",
        Kanton: lead.kanton || "",
        Zweck: lead.zweck || "",
        Branche_AI: lead.branche_ai || "",
        Datum: lead.publikation_datum || "",
        UID: lead.uid || "",
        id: lead.id
      }));
      if (statusText) statusText.textContent = `✅ ${rows.length} Leads geladen`;
    } catch (err) {
      if (statusText) statusText.textContent = "❌ Fehler beim Laden: " + err.message;
      throw err;
    }
